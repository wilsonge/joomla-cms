...
class HelperRoute
{
	...
	public function getRoute($id, $typealias, $link = '', $language = null, $catid = null)
	{
		...
		if (!empty($language) && $language != '*' && LanguageMultilang::isEnabled())
		{
			$link .= '&lang=' . $language;
	...
	protected function findItem($needles = array())
	{
		...
		if ($active && $active->component == $this->extension && ($active->language == '*' || !LanguageMultilang::isEnabled()))
		{
			return $active->id;
		}
	...
	public static function getCategoryRoute($catid, $language = 0, $extension = '')
	{
		...
		else
		{
			...
			if ($language && $language != '*' && LanguageMultilang::isEnabled())
			{
				$link .= '&lang=' . $language;
				$needles['language'] = $language;
			}

 ...
 * @since       3.4
 */
class ContactControllerFeaturedDisplay extends JControllerDisplay
{
	/*
	...
	**/
	...
	protected function getView(JModelCmsInterface $model, $prefix = null, $name = null, $type = null, $config = array())
	{
		...
		// Check if we already have a cached instance of the class
		if ($this->view[$name] instanceof $class)
		{
			return $this->view;
		}
		...
		else
		{
			...
			**/
			...
			if ($this->app->isAdmin())
			{
				$paths->insert(JPATH_ADMINISTRATOR . '/components/' . $this->config['option'] . '/view/' . $this->config['view'] . '/tmpl', 1);
			}
			else
			{
				$paths->insert(JPATH_BASE . '/components/' . $this->config['option'] . '/view/' . $this->config['view'] . '/tmpl', 1);
			}

			$view = new $class($model, $paths, $this->config);
